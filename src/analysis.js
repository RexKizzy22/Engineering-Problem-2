...
const analysis = async () => {
  ...
  const allBills = trips.map(trip => parseFloat(trip.billedAmount.toString().replace(/,/g, "")));
  const billedTotal = allBills.reduce((a,b) => {return a + b}, 0).toFixed(2);

  const cashBills = cashTrips.map(trip => parseFloat(trip.billedAmount.toString().replace(/,/g, "")));
  const cashBilledTotal = cashBills.reduce((a,b) => {return a + b}, 0).toFixed(2);

  const nonCashBills = nonCashTrips.map(trip => parseFloat(trip.billedAmount.toString().replace(/,/g, "")));
  const nonCashBilledTotal = nonCashBills.reduce((a,b) => {return a + b}, 0).toFixed(2);
  ...
  const tripsPromise = trips.map(async (trip) => {
    try {
      ...
      return {
        ...
        email: driver.email,
        phone: driver.phone,
        billedAmount: parseFloat(trip.billedAmount.toString().replace(/,/g, ""))
      }
    } catch (error) {}
  ...
  const driversPromise = uniqueDriverIDs.map(async (id) => {
    try {
      ...
      if (driver)
      vehicleAmount = Array.from(new Set(driver.vehicleID)).length;
  ...
  trips.forEach((trip) => {
    if (topDriver[trip.driverID]) {
      if (!topDriver[trip.driverID].hasOwnProperty("totalAmountEarned")) {
        amountEarned = parseFloat(trip.billedAmount.toString().replace(/,/g, ""));
  ...
  return {
    ...
    noOfDriversWithMoreThanOneVehicle: parseInt(noOfDriversWithMoreThanOneVehicle),
