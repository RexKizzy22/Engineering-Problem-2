...
async function driverReport() {
  ...
  for (let driver of uniqueTripsInfo) {
    if (!revenueByDriver[driver.driverID]) {
      ...
      if (driver.isCash) {
        let billedAmount = driver.billedAmount.toString().replace(/,/g, "");
        revenueByDriver[driver.driverID].totalCashAmount += parseFloat(billedAmount); 
        revenueByDriver[driver.driverID].noOfCashTrips += 1;
        revenueByDriver[driver.driverID].totalAmountEarned += parseFloat(billedAmount);
      } else {
        let billedAmount = driver.billedAmount.toString().replace(/,/g, "");
        revenueByDriver[driver.driverID].totalNonCashAmount += parseFloat(billedAmount); 
  ...
  uniqueTripsInfo.forEach(trip => {
    ...
    if (!driverIDs.hasOwnProperty(trip.driverID)) {
      ...
      if (trip.name) {
        ...
        driver.noOfNonCashTrips = revenueByDriver[trip.driverID].noOfNonCashTrips;
        driver.totalAmountEarned = parseFloat(revenueByDriver[trip.driverID].totalAmountEarned.toFixed(2));
        driver.totalCashAmount = parseFloat(revenueByDriver[trip.driverID].totalCashAmount.toFixed(2));
        driver.totalNonCashAmount = parseFloat(revenueByDriver[trip.driverID].totalNonCashAmount.toFixed(2));
